...
To demonstrate the implications of correlated movement on classic mean-field models epidemiological models, consider a typical \textbf{S}usceptible-\textbf{I}nfected-\textbf{R}ecovered model at the individual-level, where $p_{i, S}(t)$, $p_{i, I}(t)$, and $p_{i, R}(t)$ are the probability that individual $i$ is in the S, I, or R state at time $t$, respectively.  If individuals are moving on a gridded landscape without overlapping areas (our assumption in the main text), we can write down a set of ordinary differential equations that describe the individual-level transitions between states as
...
\end{equation}
where $\sum_{j \in N_{-i}}$ is summing over all individuals in the population excluding $i$, $\sum_{x \in Area}$ is summing over all gridded areas on the landscape, and $h^*_{i \leftarrow j}(x, t)$ is the force of infection felt by $i$ from $j$ at location $x$ at time $t$.
...
$$
p_{i, S}(t) \sum_{j \in N_{-i}} p_{j, I}(t)  \left[\frac{\beta'}{A_{tot}} + \frac{\beta'}{A_{tot}}(1 - \frac{A_x}{A_{tot}}) n \rho \right],
$$
where because space use is uniform, summing over the gridded areas is equivalent to multiplying the term within the area summation by $n$, where $n = A_{tot} / A_x$ is the number of grid cells. Note we also set $\beta' = \tilde{\beta} \lambda \pi \eta$ for notational simplicity. We can further simplify this equation to

$$
p_{i, S}(t) \frac{\beta'}{A_{tot}} \left[1 + (1 - \frac{A_x}{A_{tot}}) n \rho \right] (I - p_{i, I}(t)),
$$
...
\begin{equation}
    \begin{aligned}
        \frac{d p_{i, S}(t)}{dt} &= -p_{i, S}(t) \frac{\beta'}{A_{tot}} \left[1 + (1 - \frac{A_x}{A_{tot}}) n \rho \right] [I(t) - p_{i, I}(t)] \\
        \frac{d p_{i, I}(t)}{dt} &= p_{i, S}(t) \frac{\beta'}{A_{tot}} \left[1 + (1 - \frac{A_x}{A_{tot}}) n \rho \right] [I(t) - p_{i, I}(t)] - \gamma p_{i, I}(t) \\
        \frac{d p_{i, R}(t)}{dt} &= \gamma p_{i, I}(t),
    \end{aligned}
...
\begin{equation}
    \begin{aligned}
        \frac{d S(t)}{dt} &= -\frac{\beta'}{A_{tot}} \left[1 + (1 - \frac{A_x}{A_{tot}}) n \rho \right] [S(t)I(t) - \sum_{i \in N} p_{i, I}(t) p_{i, S}(t)] \\
        \frac{d I(t)}{dt} &= \frac{\beta'}{A_{tot}} \left[1 + (1 - \frac{A_x}{A_{tot}}) n \rho \right] [S(t)I(t) - \sum_{i \in N} p_{i, I}(t) p_{i, S}(t)] - \gamma p_{i, I}(t) \\
