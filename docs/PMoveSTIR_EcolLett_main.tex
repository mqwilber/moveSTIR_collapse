...
A large portion of epidemiological theory is built upon the assumption of independent host movements, but there is very little theory that quantifies how correlated movements affect contact and transmission risk. In contrast, there is a large body of empirical work that empirically quantifies how correlated and social movements can reshape contact and transmission landscapes \citep{Kjaer2008,Grear2010,Schauber2015a}. In general, we lack a way to quantify the role of social interactions on spatio-temporal force of infection, which limits our ability to assess when correlated movements could influence infection risk significantly. A major of goal of this study is to determine how correlated movements affect spatio-temporal infection risk for directly and indirectly transmitted pathogens.
...
Here, we develop a model we refer to as Probabilistic MoveSTIR (PMoveSTIR) to estimate epidemiological contact and expected force of infection across space and time from UDs. Our approach provides probabilistic, spatio-temporal contact networks that can be used to predict out-of-sample transmission risk and allow us to rigorously answer the question: how does among animal correlation shape spatial-temporal transmission dynamics? We derive a general model for transient, spatially heterogeneous UDs, and provide modifications to consider stationarity in space or time.
...
\end{equation}
where $\sigma_i(x) = \sqrt{p_i(x)(1 - p_i(x))}$  and $\sigma_j(x) = \sqrt{p_j(x)(1 - p_j(x))}$ are the standard deviation in probability of host $i$ and $j$ using location $x$, respectively.  Importantly, this shows that the expected force of infection between two individuals is a result of shared space use and how how correlated host movements are in location $x$ at different time lags.  For clear biological intuition, the correlation term in equation \ref{eq:stationary_cor} is directly related to the probability that host $i$ is in location $x$ given host $j$ is location $x$. 
...
Second, we used simulations to explore how temporally correlated movements affect  transmission, and how the pairwise FOI estimate depends on epidemiological parameters such as contact distance and parasite survival. In equation \ref{eq:stationary_cor}, the correlation component is entirely dependent on the observed trajectories and is therefore susceptible to the amount of data available. We explored the implications of this by simulating different tracking times. We focus on the lower-right corner of the PMoveSTIR box (equation \ref{eq:stationary_cor}), where we assume statistical stationarity in movement. The process for calculating the FOI across the landscape is summarized in Fig. \ref{fig:steps}.
...
To gain analytical intuition into the role that correlated movement can have on FOI, consider the situation where where movement is statistically stationary and hosts use space uniformly (equation S9).  %As we showed above, when correlation in movement is zero, equation \ref{eq:uniform_stationary1} reduces to mass action transmission. 
