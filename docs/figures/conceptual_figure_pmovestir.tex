...
\begin{tikzpicture}
    ...
    \node[at=(ll.south), below, font=\tiny, align=center] {Uniform space use,\\stationary time \\ (Equation S11)};
    ...
    \draw[fill=gray] (-1*\scale*0.5, -1*\scale) circle  [radius=\rad*0.5 cm] node[above, font=\tiny, align=center] (hro) {\emph{Stationary home range}\\\emph{overlap} \\ \emph{(Equation S12)}};

    \draw[fill=gray] (-1*\scale*0.5, -1*\scale*0.5) circle  [radius=\rad*0.5 cm] node[above, font=\tiny, align=center] (hro) {\emph{Time-varying home range}\\\emph{overlap} \\ \emph{(Equation S13)}};
