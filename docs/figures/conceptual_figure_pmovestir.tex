...
\begin{tikzpicture}
    ...
    \node[at=(lr.south), below, font=\tiny, align=center] {Heterogeneous space use,\\ stationary time \\ (Equation 5, 6)};
    \node[at=(ll.south), below, font=\tiny, align=center] {Uniform space use,\\stationary time \\ (Equation 7)};
    ...
    \draw[fill=gray] (-2*\scale*0.5, -1*\scale) circle  [radius=\rad*0.5 cm] node[above, font=\tiny, align=center] (hro) {\emph{Stationary home range}\\\emph{overlap} \\ \emph{(Equation 8)}};

    \draw[fill=gray] (-2*\scale*0.5, -1*\scale*0.5) circle  [radius=\rad*0.5 cm] node[above, font=\tiny, align=center] (hro) {\emph{Time-varying home range}\\\emph{overlap} \\ \emph{(Equation 9)}};

    \draw[fill=gray] (-2*\scale*0, -1*\scale*0.5) circle  [radius=\rad*0.5 cm] node[above, left, font=\tiny, align=center] (hro) {\emph{Time-varying UD}\\\emph{overlap} \\ \emph{(Equation 9)}};
