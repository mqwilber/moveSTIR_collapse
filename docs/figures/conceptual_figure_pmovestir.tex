...
\begin{tikzpicture}
    ...
    \node[at=(lr.south), below, font=\tiny, align=center] {Heterogeneous space use,\\ stationary time \\ (Equations 6, 7)};
    ...
    \draw[fill=gray] (-1*\scale*0.5, -1*\scale) circle  [radius=\rad*0.5 cm] node[above, font=\tiny, align=center] (hro) {\emph{Stationary home range}\\\emph{overlap} \\ \emph{(Equation SX)}};

    \draw[fill=gray] (-1*\scale*0.5, -1*\scale*0.5) circle  [radius=\rad*0.5 cm] node[above, font=\tiny, align=center] (hro) {\emph{Time-varying home range}\\\emph{overlap} \\ \emph{(Equation SX)}};
