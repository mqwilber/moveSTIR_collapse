...
>>>>>>> a8d931f7dc7c9d2b83029a98df67385f4998d0ec
...
As the area $A_x$ in which an epidemiological contact can occur gets smaller relative to the total area in which the hosts are moving $A_{tot}$, the correlated movement can have an orders of magnitude larger contribution to direct transmission FOI than habitat overlap at the scale of the area $Ax$. If there is significant correlation across multiple areas this will add up into a significantly greater FOI across the entire landscape.
...
The scenario is two hosts moving back and forth between two patches, and spending $\eta$ time units in each patch.  As with the example of direct transmission, we assume that the hosts are moving together. If we plot $Cor(\delta_{i \in A_x}, \delta_{j \in A_x} | s)$ of this movement scenario, we get the saw-tooth pattern observed in Fig. \ref{fig:xcorrs}a.  
...
The overall FOI varied more than two orders of magnitude among simulations, in response to both movement and epidemiological parameters. 
% Movement
...
Generally, greater interaction strengths led to higher overlap among home ranges. Simulations with interaction strengths greater than 0.9 all resulted in overlap values greater than 0.99, meaning the utilization distributions were always virtually identical. Pairs moving independently had lower overlap values on average but also higher variance, so in many cases had similarly high (\gt0.9) overlap. This allows to compare the FOI while teasing apart the effects of spatial overlap and temporal correlation in space use. 
The FOI was higher for pairs with higher interaction strengths, i.e. pairs that were more attracted to each other. This increase was due in part to higher spatial overlap, as can be seen from the higher FOI values calculated using only the stationary utilization distribution component (Fig.\ref{fig:simresults}a. However, we saw a greater increase in FOI when we accounted for temporal correlation, and the difference between the two increased with higher interaction strengths. For very similar trajectories (interaction \gt0.9), the FOI with correlation was more than five times the FOI estimated without correlation, and there was a more than 100-fold difference for perfectly overlapping trajectories.
Methods that ignore temporal correlation could therefore be significantly underestimating transmission risk. Even pairs that move independently or with low attraction could get a significantly higher FOI due to correlation; in those cases we calculated values as much as 3 times greater when accounting for correlation for highly overlapping pairs (Fig.\ref{fig:simresults}d). For pairs with low overlap, on the other hand, correlation did not play a significant role. 
Accounting for correlation usually increased the FOI with respect to an assumption of independent movement. There were, however, instances where the effect of correlation was negative and actually decreased the estimated FOI. For some parameter combinations this could happen in as many as half of the simulations of independent movement (Fig.\ref{fig:simresults}d), and even for 10\% of simulations with interaction strength of 0.7. The decrease was nonetheless small compared to simulations where the FOI was greater with than without covariance, the greatest decrease we observed was 15\%. 

% Epidemiological parameters
%We also analyzed how changes in epidemiological parameters, namely the parasite decay rate and the contact distance, influenced the estimated risk of transmission for a given pair of individuals. 
The FOI was inversely proportional to the decay rate; if parasites survived longer in the environment this led to higher estimated FOI (Fig. \ref{fig:simresults}b). This increase is driven mostly by the linear increase in the spatial overlap component ($p_i(x)p_j(x)/\nu$), for longer decay times the relative importance of the correlation term decreased exponentially (Fig.\ref{fig:simresults}e). Longer survival times increase the probability of indirect epidemiological contact, but also give greater importance to correlations at longer time lags, which in many cases can be trivial or even negative  (Fig\ref{fig:xcorrs}). Thus, while correlation can increase the estimated FOI, this effect is greatest for parasites with short survival times. 

The FOI in general increased as parasites persisted longer in the environment. In contrast, the effect of contact distance varied depending on interaction strength. For pairs with low interaction there was no noticeable effect of increasing the threshold contact distance (Fig. \ref{fig:simresults}c). However, for  similar trajectories (interaction \gt 0.9), FOI increased between two and five-fold as contact distance increased, while for identical trajectories (interaction = 1) FOI decreased. This effect is driven by changes in the contribution of the correlation term (Fig.\ref{fig:simresults}f). For highly similar trajectories, for example (interaction = 0.96), the FOI estimated with correlation was around ten times greater than the FOI with only the spatial overlap component for the smaller cells, but as much as 100 times greater for the larger cells.
Increasing the contact distance increases the probability of finding two individuals in the same cell, and would therefore increase the relative contribution of correlation most for trajectories that are already close in space and time (Fig.\ref{fig:simresults}f), while it would have no effect for trajectories that are far from each other. For identical trajectories, however, grouping observations into larger cells imply fewer cells where correlation is impacting the FOI, and can thus reduce the contribution of correlation and the overall FOI. 

\subsection*{Empirical example: white-tailed deer}

Deer in our study varied in overlap among their home ranges, between 1\% and 91\% (Bhattacharyya coefficient). The total pairwise FOI also varied significantly, almost four orders of magnitude between pairs (range: 7.6 x 10^{-9} s^{-1} -- 8.5 x 10^{-5}s^{-1}). This was mostly due to one pair in particular which had a significantly higher FOI than the rest. 
While higher contributions of the correlation term increased the FOI, the relative contribution was small compared to the importance of the spatial overlap component. The highest contribution was in the order of 0.4\% of the FOI value. In some cases the correlation actually decreased the FOI with respect to using just the UD component, but again only by 0.1\%. 
The FOI estimated using PMoveSTIR was correlated with the FOI estimated using the overlap of the home ranges, but produced higher as well as lower values. The difference between the two did not seem to be related to the role of covariance or to the spatial overlap component. % maybe they do but the effect is really marginal.
The relatively low importance of covariance in this empirical example, compared to the simulation results, is possibly related to the relative cell size. While in simulations the smallest cells represented 0.002\% of the home range area, in the empirical example cells represent at most 0.009\% of the individual's home range. 
