...
In the simplest case, individuals are moving independently of each other, so the correlation between their visits at any given location is zero. The FOI is simply $h_{i\leftarrow j}(x)=\frac{\tilde\beta}{\nu A_x} p_i(x)p_j(x)$. 
This is equivalent to having an identity matrix as the social kernel in our movement model.
