...
The magnitude and relative effect of correlation on FOI are modulated by epidemiological factors such as parasite transmission and survival in the environment, and epidemiological contact distance. Longer decay times (i.e. lower decay rates) resulted in  a lower contribution of correlation to FOI. Biologically this means that if a parasite can persist for long in the environment it does not matter how long it takes for a susceptible host to visit a cell that an infected host visited previously. Here, we have made two key assumptions about the  environmental transmission stages of parasites: that they can infect a new host immediately following deposition, and that they decay at a constant rate. Under this assumption, correlations at short lags have the greatest impact on the overall FOI, while considering longer lags results in a reduction of the contribution of correlation to FOI, as we saw for CWD. The PMoveSTIR framework nevertheless allows to use any type of survival function. Many helminths, for example, have delayed transmissibility due to a development period. Our results therefore highlight the importance of understanding the biology of parasites during free-living stages, and how their development and survival is affected by environmental conditions. Given the delay in transmission in these systems, we would expect correlation to play only a minor role, and that the indirect encounter probability would be accurately captured by the spatial overlap component of PMoveSTIR. This expectation should nonetheless be tested with system-specific parameter estimates.  
% Decay function
The epidemiological threshold contact distance, which is reflected in our workflow as the side of the grid cells, also influenced the estimated FOI, although this effect was only clearly noticeable in two particular cases: when trajectories were perfectly identical, and when they were nearly identical. Larger grid cells should result in a reduction of the local FOI, as it would take longer to find a packet of pathogen across a larger area, but it also increases the probability of two individuals being in the same cell. This would explain the marked increase in FOI we observed for nearly identical trajectories, driven by increased contribution of correlation. Depending on the proximity of hosts in empirical applications, this definition of distance could therefore have a significant effect on FOI estimation, and special attention should be given to behaviors that drive transmission at small scales.
...
Because these correlation occur at particular locations, they could potentially be associated with local ecological characteristics. In our results, correlations substantially increased heterogeneity in the landscape of risk; strong correlations at specific cells can significantly increase or decrease the local FOI, while having no effect in adjacent cells if they weren't visited. While we are using a probabilistic description of space use, the correlation term in PMoveSTIR is still dependent on observations, and its effect will therefore vary significantly across space \citep{Wilber2022}. This also implies that the estimation of FOI could vary substantially depending on the duration of data observed; both encounters at new locations and repeated use of the same locations can alter local correlation patterns.  
...
% Concluding remarks
We have focused on one special case of the PMoveSTIR space that can be applied to stationary utilization distributions, which we think could be the most readily applicable framework for empirical tracking data. However, the general framework can be adapted to make use of different data streams, such as proximity loggers or camera-traps \citep{Wilber2022}. 
