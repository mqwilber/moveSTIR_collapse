...
To calculate the FOI between pairs of individuals we first determine where they may come into contact, directly or indirectly. We determine the areas of probable contact using the conditional distribution of encounters. The instantaneous encounter rate is calculated as (Noonan et al. 2021)
...
This is interesting for our purposes because the numerator--the product of the utilization distributions--is the same thing as the first term in our FOI calculation. Given an estimated CDE we could obtain the UD product multiplying by the denominator. We could also obtain it simply by multiplying the UDs together, provided they are on a common discrete grid. For this product we need the probability mass function, i.e. the total probability of being in a given *area*. Kernel density estimation functions output the probability *density*, so we need to scale this multiplying by the grid cell area first. 
...
<!-- I probably don't need to get the CDEs, I can calculate the product of the UDs separately, and use that to get the area to sample in -->
...
For the grid approach, we simply take the grid produced by the pairwise CDEs. The problem with this is that there could be a mismatch between this scale and the threshold distance for epidemiological contact. This distance is usually small, in the order of a meters.

```{r corr-grid approach}
# get the rasters for the CDEs. We choose the probability mass function here,
# otherwise we have to scale by the grid area
CDErasters <- lapply(CDEs, raster, DF = "PMF")
...
We multiply the UDs pairwise to get the probability $p_i(x)p_j(x)$. This is the first component of the equation. We calculate the standard deviation components also from the UD probabilities. The per-cell standard deviation is $\sigma=\sqrt{\hat p(1-\hat p)}$, where $\hat p$ is the estimated probability for a binomial process (i.e. the UD PMF). Ideally, the probability of contact should be calculated at a scale relevant for epidemiological contact, which for large mammals is in the order of meters. We can, however, use the values estimated at a larger scale, provided we scale the density function to the same area as the correlation calculation.
...
for (i in seq_along(CDErasters)) {
  ...
  udprob <- r1*r2
  UDprods[[paste(ind1,ind2,sep = "-")]] <- udprob/cellarea
  sdprob <- sqrt(r1*(1-r1))*sqrt(r2*(1-r2))
  UDsds[[paste(ind1,ind2,sep = "-")]] <- sdprob/cellarea
}
# duplicate rasters
...
A simple way to avoid scale mismatches is to set the scale of the discretized utilization distributions based on the threshold distance of the transmission process. That way all the calculations are performed at the same scale, which is the one relevant for transmission. 
...
cellarea2 <- prod(res(UDepirasts[[1]]))

# extend all rasters to have the same extent. Could be done inside loop
UDepirasts <- lapply(UDepirasts, extend, y = extent(do.call(merge,UDepirasts)))
# Create lists to store objects
...
for (i in seq_len(ncol(combs))) {
  ...
  # product of probabilities, divided by area
  udprob <- r1*r2
  UDepiprods[[paste(ind1,ind2,sep = "-")]] <- UDepiprods[[paste(ind2,ind1,sep = "-")]] <- udprob/cellarea2
  sdprob <- sqrt(r1*(1-r1))*sqrt(r2*(1-r2))
  UDepisds[[paste(ind1,ind2,sep = "-")]] <- UDepisds[[paste(ind2,ind1,sep = "-")]] <- sdprob/cellarea2
}
...
for (i in seq_along(UDepiprods)) {
  ...
  # # keep only cells where the probability is not 0. (For memory saving)
  # indx <- Which(r1>0, cells = TRUE)
  # # raster resolution and center coords
  # yr <- yres(r1)
  # xr <- xres(r1)
  # gridcenters <- coordinates(r1)[indx,]
  # posindex1 <- abs(outer(xsoc[,ind1], gridcenters[,1],"-"))<=xr/2 &
  #   abs(outer(ysoc[,ind1], gridcenters[,2],"-"))<=yr/2
  # posindex2 <- abs(outer(xsoc[,ind2], gridcenters[,1],"-"))<=xr/2 &
  #   abs(outer(ysoc[,ind2], gridcenters[,2],"-"))<=yr/2
  ...
  if(length(ovlpcells)==0) {
    ...
    gridcors[[paste(ind1,ind2,sep = "-")]] <- gridcors[[paste(ind2,ind1,sep = "-")]] <- NA
    message("There are no overlap cells between", ind1, "and", ind2,", moving on to next pair")
    next
  }
  # # keeping only cells with actual overlap of data, for memory saving
  ...
  # posindex2 <- posindex2[,ovlpcells]
  # 
  cormat_ab <- cormat_ba <- matrix(0,nrow = nsteps-1, ncol = length(ovlpcells))
  for (j in ovlpcells) {
    maxlag <- nsteps-1
    a <- b <- numeric(maxlag)
    a[match(j, pos1)] <- b[match(j, pos2)]<- 1
    xcorr <- ccf(a,b,lag.max = maxlag, plot = F)
    xcorr_vals <- as.numeric(xcorr$acf)
    cormat_ab[,j] <- rev(xcorr_vals[1:nsteps])
    cormat_ba[,j] <- xcorr_vals[nsteps:length(xcorr_vals)]
