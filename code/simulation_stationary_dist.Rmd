...
# import back
gridcors <- readRDS("../data/example_correlations.rds")
```

The other alternative is to sample randomly only inside the area where encounters are likely. We take the 95% polygon of the CDE, sample grid cells inside it and estimate the correlation for every pair in a cell around each sampled point. The grid cell size should be the same as the one for the CDE grid. For this to make sense we would need a higher density of points than with the regular grid, so the number of sampled points should be a function of the total area of overlap. With this example--6 individuals, 1000 steps--and at a density of 1 point per cell it took less than 10 minutes.

```{r randompt-approach}
# select random points within the overlap polygons
CDE_ovlpareas <- sapply(CDEpolys,raster::area)[3,]
CDE_cellsizes <- sapply(CDErasters, \(x) prod(res(x)))
CDEpolys_up <- lapply(CDEpolys, \(x) polygons(x)[3]) # sample only the wider 95% estimate area
pts_percell <- 3
CDE_n_samplepts <- pts_percell*CDE_ovlpareas/CDE_cellsizes
samplepts <- lapply(seq_along(CDEpolys_up), \(x) spsample(CDEpolys_up[[x]], n = CDE_n_samplepts[x], type = "random")) |> lapply(as.data.frame)
names(samplepts) <- names(CDEpolys_up)
ptcors <- list()
lagmat <- outer(seq_len(nsteps), seq_len(nsteps), "-")
lags <- round(unique(lagmat[lower.tri(lagmat,diag = TRUE)]))
lags <- lags[-length(lags)]
# Calculate the correlations for all lags at each point
tictoc::tic(); for (i in seq_along(samplepts)) {
  sppts <- samplepts[[i]]
  indpair <- names(samplepts)[i]
  ind1 <- as.numeric(substr(indpair, 1,1))
  ind2 <- as.numeric(substr(indpair, 3,3))
  xr <- xres(CDErasters[[i]])
  yr <- yres(CDErasters[[i]])
  posindex1 <- abs(outer(coordinates(sppts)[,1],xs[,ind1],"-"))<=xr/2 & abs(outer(coordinates(sppts)[,2],ys[,ind1],"-"))<=yr/2
  posindex2 <- abs(outer(coordinates(sppts)[,1],xs[,ind2],"-"))<=xr/2 & abs(outer(coordinates(sppts)[,2],ys[,ind2],"-"))<=yr/2
  cormat <- matrix(0,nrow = length(lags), ncol = nrow(sppts))
  tictoc::tic("lag"); for (s in seq_along(lags)) {
    l <- lags[s]
    matindices <- which(lagmat==l, arr.ind = TRUE)
    h1 <- t(posindex1[,matindices[,1]])
    h2 <- t(posindex2[,matindices[,2]])
...
for (i in seq_along(CDErasters)) {
  # par(mfrow = c(1,2))
  c1 <- colSums(gridcors[[i]]*exp(-lags))
  indpair <- names(CDErasters)[i]
  ...
  c2 <- colSums(ptcors[[i]]*exp(-lags))
  corvalsscaled <- (c2-min(c2))/(2*max(abs(range(c2))))
  ...
  lines(xs[,ind1],ys[,ind1],pch = 16, col = hcl.colors(nind, "Set 2",alpha = 0.5)[ind1])
  lines(xs[,ind2],ys[,ind2],pch = 16, col = hcl.colors(nind, "Set 2",alpha = 0.5)[ind2])
  points(samplepts[[i]], col = corcols, pch = 16)
  # legend("bottomleft", fill = hcl.colors(5, "Blue-Red"),legend = seq(max(abs(range(c2))),-max(abs(range(c2))),length.out = 5))
}
```
