...
For the grid approach, we simply take the grid produced by the pairwise CDEs. 
...
for (i in seq_along(CDErasters)) {
  ...
  for (j in 1:ncol(posindex1)) { 
    ...
    cormat_ab[,j] <- xcorr_vals[1:nsteps]
    cormat_ba[,j] <- rev(xcorr_vals[nsteps:length(xcorr_vals)])
  }
