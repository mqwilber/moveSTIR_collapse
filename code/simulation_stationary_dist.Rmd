...
pairfoi <- sapply(foirasts2, cellStats, sum)
foi_pair_df <- data.frame(do.call(rbind, strsplit(names(foirasts2), split = "-")),
           foi = pairfoi,
           covfoi = sapply(seq_along(foirasts2), \(x) cellStats((foirasts2[[x]]-beta/cellarea*lam*UDepiprods[[x]]/nu)/foirasts2[[x]]*100,mean)))
...
The $\nu$ parasite decay parameter can have a substantial impact on the overall FOI. 
```{r foi v nu}
nus <- 1/2^(0:10)
