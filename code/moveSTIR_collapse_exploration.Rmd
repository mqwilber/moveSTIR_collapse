...
Here I will use simulated tracks following the approach by Hooten and
Johnson [@hooten2017]. This is also the method used in the original
moveSTIR paper. I can use the same tracks first, and then make changes
to test different case studies. I should try out:

-   Different levels of overlap

-   Correlated movement (see [@scharf2018] for joint movement)

```{r}
# import the data simulated in the original moveSTIR paper. The tracks are csv files generated using the moveSTIR tutorial notebook, and the simulate_movement function in moveSTIR.
sim_tracks <- lapply(list.files("../data", ".csv",full.names = TRUE), read.csv)
library(sp)
...
grid_cell_coords <- SpatialPointsDataFrame(coordinates(kuds[[1]]), data = data.frame(kuds[[1]]@grid.index))
# Get the distance between the grid cell centers and every point in the tracking history. THis creates a numeric matrix with points in rows, and grid cell centers in columns. We need only keep the lowest distance.
point_grid_dists <- spDists(sim_tracks_sppoints, grid_cell_coords)
nearest_cell <- apply(point_grid_dists, MARGIN = 1, which.min)
loc_history <- matrix(0, nrow = nrow(sim_tracks[[1]]), ncol = length(kuds[[1]]))
for (i in nrow(loc_history)) {
  loc_history[i,nearest_cell[i]] <- 1
}
# calculate standard deviation. For a Bernoulli process, the sd is sqrt(p(1-p)). p is just the column mean for the location history matrix.
sig <- loc_history |> apply(2,\(x) sqrt(mean(x)*(1-mean(x))))
# Calculate the per-cell product of expected values, and plot it
SpatialPixelsDataFrame(coordinates(kuds[[1]]), data = data.frame(kuds[[1]]@data*kuds[[2]]@data)) |> plot()
...
for (i in unique(sim_tracks_db$ind)) {
  lines(sim_tracks_db[sim_tracks_db$ind == i, ], col = hcl.colors(6, palette = "Set 2")[i], lwd  = 2)
}
legend(x = "bottomright",legend = 1:6, col = hcl.colors(6,palette = "Set 2"),border = F, lty = 1)
...
Draw the joint probability distribution grid
