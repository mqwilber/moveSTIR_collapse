...
Finally, given stationarity, we know that
$Cov(\delta_i(x, t), \delta_j(x, u))$ just depends on the the time lag
$t - u$ and not specific time stamps, so we are really just focusing on
$Cov(\delta_i(x), \delta_j(x) | t - u)$. Thus, we can express the
equation as
...
$$ where - $\rho(\delta_i(x),\delta_j(x))$ is the correlation between
the occurrences of $i$ and $j$ at $x$, and - $\sigma(\delta_i(x))$ is
the standard deviation of the occurrences of $i$ at $x$. We obtain $$
...
1.  If movement is not independent, we need to determine the conditional
    ...
    special case of direct contact, $t=u$, this is simply the
...
2.  Knowing the lagged versions of
    ...
    if we had environmental loggers. The cool thing is, then all we have
    to do is use a logistic regression with a time-varying spline (or
...
$F<1$, so evaluating we would obtain
...
```{r, fig.cap="Conditional probability of the time lag between two individuals i and j at the same location x"}
# Define two overlapping ranges in one dimension
xmin1 <- 0
xmax1 <- 10
xmin2 <- 7
xmax2 <- 15
