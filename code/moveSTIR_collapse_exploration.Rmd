...
kernel which allows to account for joint movement. I can use the simple,
independent tracks first, and then make changes to test different case
studies.
...
for (i in unique(sim_tracks_db$ind)) {
  lines(sim_tracks_db[sim_tracks_db$ind == i, ], col = hcl.colors(5, palette = "Set 2")[i], lwd  = 2)
}
legend(x = "bottomright",legend = 1:5, col = hcl.colors(5,palette = "Set 2"),border = F, lty = 1)
```
...
cell_covs <- replicate(n = nrow(ind_combs), 
                       expr = matrix(nrow = length(lags), 
                                     ncol = ncol(loc_histories[[1]])),
                       simplify = FALSE)
cell_cors <- replicate(n = nrow(ind_combs), 
                       expr = matrix(nrow = length(lags), 
                                     ncol = ncol(loc_histories[[1]])), 
                       simplify = FALSE)
...
for (r in 1:nrow(ind_combs)) {
  i <- ind_combs[r,2]
  j <- ind_combs[r,1]
  covmat <- matrix(nrow = length(lags), ncol = ncol(loc_histories[[1]]))
  cormat <- matrix(nrow = length(lags), ncol = ncol(loc_histories[[1]]))
  tic("comb")
  for (l in lags) {
    # l <- lags[s]
    # find position where those lags occur
    ...
    tic("cov")
    covmat[s,] <- diag(cov(h1,h2)) # only the diagonal as we care only about same cells
    toc(log = T, quiet = T)
    tic("cor")
    cormat[s,] <- diag(cor(h1,h2)) # only the diagonal as we care only about same cells
    toc(log = T, quiet = T)
  }
  cell_covs[[r]] <- covmat
  cell_cors[[r]] <- covmat
...
# for (i in seq_along(loc_histories)) {
#   for (j in seq_along(loc_histories)) {
    # if (i == j) next
    # # exclude cells that only one individual visited
    # visitprod <- colSums(loc_histories[[i]])*colSums(loc_histories[[j]])
    # ovlpcells <- which(visitprod!=0)
    # if(length(ovlpcells)>0) {
    #   cellcorrs <- rbind(cellcorrs, cbind(ind1=i,ind2=j,cell = goodcells, cor = r))
    # }

#     names(cell_covs)[counter] <- paste(i,j)
#     counter = counter+1
#   }
# }

# multiply by the decay, and sum across 
cell_covsum <- lapply(cell_covs, "*", exp(-nu*lags)) |> 
  lapply(colSums, na.rm = TRUE)
```
...
```{r foi}
UDmu <- lapply(UDprods, "*", 1/nu)
cell_foi <- mapply("+", cell_covsum, UDmu) |> sapply("*", beta*lam)
```
...
```{r tracks-gg}
sim_tracks_db %>% ggplot()+geom_path(aes(x,y,color = factor(ind)), size=1)+theme_void()+coord_sf()+labs(color="Individual")+theme(legend.position = "left") -> gg.tracks
```

```{r pairwise-foi}
# total FOI from for every pair of individuals
colSums(cell_foi) |> cbind(ind_combs) |> ggplot()+geom_raster(aes(factor(ind1),factor(ind2),fill=colSums(cell_foi)))+theme_classic(base_size = 14)+labs(fill = "FOI", y = "Acquiring individual", x = "Depositing individual")+scale_x_discrete(breaks = 1:6)+scale_y_discrete(breaks = 1:6)+theme(aspect.ratio = 1) -> gg.pairfoi
cowplot::plot_grid(gg.tracks,gg.pairfoi, rel_widths = c(1,1.3))
```

We can see for example that individual 1 has hardly any overlap, and its
FOI values are the lowest. The FOI in this case is symmetrical. The
product of the UDs is symmetrical, and the covariance is zero for all
pairwise interactions. This makes sense because there is no real
interaction across hosts.

```{r ind-foi}
# total FOI experienced across from all hosts combined
tapply(colSums(cell_foi), ind_combs$ind1, sum)
# total FOI contributed to all other hosts
tapply(colSums(cell_foi), ind_combs$ind2, sum)
```
